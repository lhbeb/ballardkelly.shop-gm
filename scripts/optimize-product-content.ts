...
async function main() {
  ...
  if (exportData.products.length !== 25 || missing.length || unknown.length) {
    throw new Error(`Expected the 25-product BallardKelly catalog. Found ${exportData.products.length}; missing: ${missing.map((p) => p.slug).join(', ') || 'none'}; unknown: ${unknown.join(', ') || 'none'}.`);
  }
  ...
  console.log(`Optimized ${updated} of ${exportData.products.length} BallardKelly products.`);
}
