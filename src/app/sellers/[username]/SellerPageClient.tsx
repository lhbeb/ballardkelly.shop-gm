...
export default function SellerPageClient({ seller }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[#f2f7f4] flex flex-col font-sans">
      ...
      {/* Hero Section */}
      <div className="bg-gradient-to-br from-[#1f5a46] to-[#1f5a46] text-[#f2f7f4] pt-24 pb-12 rounded-b-[40px] shadow-lg relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('/grid-pattern.svg')] opacity-10"></div>
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#5f8f7a] rounded-full filter blur-[120px] opacity-10 -translate-y-1/2 translate-x-1/2"></div>
        
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-8">
            
            {/* Avatar */}
            <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-[#f2f7f4]/20 overflow-hidden flex-shrink-0 bg-[#f2f7f4]/5 relative z-20 shadow-2xl">
              {seller.avatarUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={seller.avatarUrl} alt={seller.name} className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full bg-gradient-to-br from-[#1f5a46] to-[#1f5a46] flex items-center justify-center text-5xl font-bold text-[#f2f7f4]">
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="flex flex-col md:flex-row items-center md:items-end gap-3 mb-2">
                ...
                <div className="relative group flex">
                  <div className="cursor-help flex items-center gap-1.5 bg-[#5f8f7a] text-white px-2.5 py-1 rounded-full text-xs font-semibold backdrop-blur-sm border border-[#5f8f7a]/50 transition-colors hover:bg-[#5f8f7a]/90">
                  ...
                  <div className="absolute top-full mt-3 left-0 md:left-auto md:right-auto w-72 p-4 bg-white text-gray-600 text-sm leading-relaxed rounded-2xl shadow-2xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 transform origin-top translate-y-2 group-hover:translate-y-0">
                    <div className="font-bold mb-1.5 flex items-center gap-1.5 text-[#262626]">
                      <Star className="w-4 h-4 text-[#1f5a46] fill-[#1f5a46]" /> Star Seller
                    </div>
              ...
              <p className="text-[#f2f7f4]/75 font-medium mb-4 flex items-center justify-center md:justify-start gap-2">
                @{seller.username}
              </p>
              
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-y-2 gap-x-6 text-sm text-[#f2f7f4]/80 mb-6">
                {(seller.location || 'United States') && (
                  <div className="flex items-center gap-1.5">
                    <MapPin className="w-4 h-4 text-[#5f8f7a]" />
                    <span>{seller.location || 'United States'}</span>
                  </div>
                )}
                <div className="flex items-center gap-1.5">
                  <Calendar className="w-4 h-4 text-[#5f8f7a]" />
                  <span>Joined {seller.memberSince || new Date(seller.createdAt || Date.now()).getFullYear()}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <Package className="w-4 h-4 text-[#5f8f7a]" />
                  <span>{products.length} Items Listed</span>
                </div>
              </div>

              {seller.bio && (
                <div className="bg-[#f2f7f4]/10 p-4 rounded-2xl backdrop-blur-md border border-[#f2f7f4]/15 text-[#f2f7f4]/90 text-sm md:text-base leading-relaxed max-w-2xl">
                  {seller.bio}
                </div>
              )}
                    ...
                    {[...Array(5)].map((_, i) => (
                        ...
                        className={`h-4 w-4 ${
                          i < Math.floor(seller.averageRating ?? 0)
                            ? 'text-[#5f8f7a] fill-[#5f8f7a]'
                            : 'text-white/30'
                        }`}
                      />
                    ))}
                  </div>
                  <span className="text-[#f2f7f4]/85 text-sm font-medium">
                    {(seller.averageRating ?? 0).toFixed(1)}
                  </span>
                  <span className="text-[#f2f7f4]/60 text-sm">
        ...
        <div className="container mx-auto px-4 py-12">
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="w-full lg:w-1/3 xl:w-1/4 space-y-6 lg:sticky lg:top-24">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-[#1f5a46]/10">
                <h2 className="text-lg font-bold text-[#262626] mb-4 flex items-center gap-2">
                  <ShieldCheck className="w-5 h-5 text-[#1f5a46]" />
                  Seller Policies
                </h2>
              ...
              <div className="bg-gradient-to-br from-[#1f5a46] to-[#1f5a46] p-6 rounded-3xl border border-[#1f5a46]/10 flex flex-col items-center text-center text-[#f2f7f4] shadow-sm">
                <div className="w-12 h-12 bg-[#5f8f7a]/15 text-[#5f8f7a] rounded-full flex items-center justify-center mb-3">
                  <HelpCircle className="w-6 h-6" />
                </div>
                <h3 className="font-semibold text-[#f2f7f4] mb-2">Have a question?</h3>
                <p className="text-sm text-[#f2f7f4]/80 mb-4">You can reach out to our dedicated support team regarding any items sold by {seller.name}.</p>
                <Link href="/contact" className="w-full px-4 py-2.5 bg-[#5f8f7a] text-white font-medium rounded-xl hover:bg-[#5f8f7a]/90 transition-colors shadow-sm text-sm">
                  Contact Support
                </Link>
              </div>
            </div>
            ...
            <div className="w-full lg:w-2/3 xl:w-3/4 flex flex-col min-h-[50vh] gap-12">
              ...
              <div>
                ...
                {loading ? (
                    ...
                    {totalPages > 1 && (
                      <div className="mt-10 flex items-center justify-center gap-2">
                        <button
                          ...
                          className="flex items-center justify-center w-10 h-10 rounded-full border border-[#1f5a46]/15 bg-white text-gray-600 hover:bg-[#f2f7f4] hover:text-[#1f5a46] disabled:opacity-50 disabled:hover:bg-white disabled:hover:text-gray-600 transition-colors"
                            ...
                            <button
                              ...
                              className={`w-8 h-8 rounded-full text-sm font-medium transition-colors flex items-center justify-center ${
                                page === '...'
                                  ...
                                  : currentPage === page
                                  ? 'bg-[#1f5a46] text-[#f2f7f4]'
                                  : 'text-gray-600 hover:bg-[#f2f7f4] hover:text-[#1f5a46]'
                              }`}
                            >
                              {page}
                            </button>
                  ...
                  <div className="flex-1 bg-white rounded-3xl border border-[#1f5a46]/10 flex flex-col items-center justify-center p-12 text-center">
