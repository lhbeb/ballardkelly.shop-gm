    ...
    <div className={`${cardBackground} rounded-md shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col`}>
        ...
        <div className={`relative w-full ${imageBackground} ${showFullImage ? 'aspect-square' : 'h-48'}`}>
          ...
          {isSoldOut && (
            <div className="absolute inset-0 bg-[rgba(0,0,0,0.65)] flex items-center justify-center rounded-t-md">
              <div className="bg-[#c8942f] rounded-lg px-5 py-2">
                <span className="sold-out-badge text-[#1f5a46] text-sm uppercase tracking-wider whitespace-nowrap font-bold">
                  Sold Out
                </span>
              </div>
            </div>
      ...
      <div className="p-4 flex-grow flex flex-col">
        ...
        <div className="mt-auto pt-3 flex flex-col gap-2">
          ...
          <Link
            href={`/products/${slug}`}
            className="flex items-center text-sm text-[#1f5a46] hover:text-[#102820] transition-colors"
          >
